package ClassePergunta;

use strict;
use ClasseUsuario;
use ClassePerguntaInfo;
use FuncoesUteis;
...
#--------------------------------------------------------------------------------
# Classe para criar e manipular objetos que controlam perguntas
#################################################################################
   ...
   if (@_) {
      ...
      if (length($titulo) > 0) {
         if (length($titulo) <= 60) {
            ...
            die "Título da pergunta pode conter no máximo 60 caracteres!\n";
         }
      } else {
...
sub setTexto {
   ...
   if (@_) {
      ...
      die "Texto da pergunta está inválido!\n";
   }
}
#--------------------------------------------------------------------------------
sub getTexto {
...
sub setData {
   ...
   if (@_) {
      ...
      die "Data da pergunta está inválida!\n";
   }
}
#--------------------------------------------------------------------------------
sub getData {
...
sub setDataLimite {
   ...
   if (@_) {
      ...
      die "Data limite da pergunta está inválida!\n";
   }
}
#--------------------------------------------------------------------------------
sub getDataLimite {
...
sub setDataDivulgacao {
   ...
   if (@_) {
      ...
      die "Data divulgação da pergunta está inválida!\n";
   }
}
#--------------------------------------------------------------------------------
...
# info
#--------------------------------------------------------------------------------
# métodos complementares de informações sobre a pergunta
#################################################################################
sub info {
   my $self = shift;
   if (@_) {
...
# 0-codigoPergunta, 1-titulo, 2-texto, 3-data, 4-dataLimite, 5-habAnexo,
# 6-valorNota, 7-codigoUsuario, 8-dataDivulgacao
#################################################################################
sub setDados {
   my $self = shift;
   if (@_) {
      $self->setCodigoPergunta($_[0]);
      $self->setTitulo($_[1]);
      $self->setTexto($_[2]);
      $self->setData($_[3]);
      ...
      die "Nenhum argumento foi enviado para a pergunta!";
   }
}
#################################################################################
...
sub setDadosBD {
   ...
   if (@_) {
      ...
      die "Nenhum argumento foi enviado nas informações da pergunta!";
   }
}
#################################################################################
...
# 0-codigoPergunta, 1-titulo, 2-texto, 3-dataLimite, 4-habAnexo, 5-valorNota,
